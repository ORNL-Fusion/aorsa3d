...
!
    subroutine QLSUM_MAXWELLIAN(k_uper, b_sum, c_sum, e_sum, f_sum, &
       ...
       & nphidim1, nphidim2, nkx1, nkx2, nky1, nky2, nphi1, nphi2, &
       & uxx, uxy, uxz, &
       & uyx, uyy, uyz, &
       & uzx, uzy, uzz, &
       & nxdim, nydim, nphidim, xkxsav, xkysav, xkzsav, capr, xx, yy, zz, &
       & i_global, j_global, k_global, &
       & lmaxdim, ndist, nzeta, &
       & gradprlb, bmod, omgc, alpha, xm, upshift, xkx_cutoff, &
       & xky_cutoff, xkz_cutoff, rt)

    implicit none

    integer i_global, j_global, k_global, ier, nxdim, nydim, nphidim, &
   &   k, lmaxdim, ndist
    integer, intent(IN):: NUPAR, NUPER, lmax
    integer nkx1, nkx2, nky1, nky2, nphi1, nphi2, j_upar, k_uper, l
    integer nkdim1, nkdim2, mkdim1, mkdim2, nphidim1, nphidim2
   ...
   &   descrim, xme, fgam, gradprlb, gammab, xkx_cutoff, xky_cutoff, &
   &   xkz_cutoff, rt, kr
    ...
    integer, dimension(:),  allocatable :: nphires
    ...
    complex ealphak(nkdim1 : nkdim2, mkdim1 : mkdim2, nphidim1 : nphidim2), &
    &        ebetak(nkdim1 : nkdim2, mkdim1 : mkdim2, nphidim1 : nphidim2), &
    &           ebk(nkdim1 : nkdim2, mkdim1 : mkdim2, nphidim1 : nphidim2)
    ...
    allocate( zbeta(nkx1:nkx2,nky1:nky2,nphi1:nphi2) )
    allocate( zbeta_iharm(nkx1:nkx2,nky1:nky2,nphi1:nphi2) )

    allocate(zetai(nzeta + 1) )
    allocate(Jni(-lmaxdim : lmaxdim, nzeta + 1) )
    allocate( Jn(-lmaxdim : lmaxdim, nkdim1 : nkdim2, mkdim1 : mkdim2, nphidim1 : nphidim2))
    allocate(NPARA_sav(nkdim1 : nkdim2, mkdim1 : mkdim2, nphidim1 : nphidim2) ) 
    
    allocate(nres(nxdim * nydim * nphidim) )
    allocate(mres(nxdim * nydim * nphidim) )
    allocate(nphires(nxdim * nydim * nphidim) )
    ...
    allocate(is_resonance_nm(nkx1:nkx2,nky1:nky2,nphi1:nphi2))
...
!              if (upshift .eq. 0)xkprln = uzz * xkphi
	      if (upshift .eq. 0)xkprln = xkzsav(nphi) /rt
	      
	     if (upshift .eq. -1) then
                kr = sqrt((xkxsav(n) / xkx_cutoff)**2 &
     &                    + (xkysav(m) / xky_cutoff)**2 &
     &                    + (xkzsav(nphi) / xkz_cutoff)**2)
                if (kr .gt. 1.0) xkprln = uzz * xkphi
             end if	      
...
!             if(abs(gammab) .gt. 1000.0) gammab = 1000.0
     ...
     &                / (2. * gammab * y)
             ...
             zbeta(n,m,nphi) = cmplx( cosbeta_n_m , sinbeta_n_m  )
             ...
             do IHARM = 0, NHARM + 1
                Jn(iharm,  n, m, nphi) = b(iharm + 1)
                Jn(-iharm, n, m, nphi) = (-1.0)**iharm * Jn(iharm, n, m, nphi)
             end do
...
!             if(abs(gammab) .gt. 1000.0) gammab = 1000.0
     ...
     &                / (2. * gammab * y)
             ...
             do iharm = -NHARM - 1, NHARM + 1
                ...
                Jn(iharm, n, m, nphi) = Jni(iharm, i)    &
     &   	   + p * (Jni(iharm, i + 1) - Jni(iharm, i))
                if(i .ne. 1 )then
                   Jn(iharm, n, m, nphi) = A1 * Jni(iharm, i - 1)     &
     &                             + A2 * Jni(iharm, i)         &
     &                             + A3 * Jni(iharm, i + 1)
                end if
    ...
    do IHARM = -NHARM, NHARM
       ...
       call zpow((nkx2-nkx1+1)*(nky2-nky1+1)*(nphi2-nphi1+1), zbeta, iharm, zbeta_iharm)
       ...
       do n = nkx1, nkx2
          do m = nky1, nky2     
             ...
             ! -- Resonance relation -- !
             ! ------------------------ !
             RRP = 1.0 - NWCW - NPARA_sav(n, m, nphi) * UPARMAX * SQMUT0i
             RRM = 1.0 - NWCW - NPARA_sav(n, m, nphi) * UPARMIN * SQMUT0i

             is_resonance_nm(n,m,nphi) = (RRP * RRM .le. 0.0)
             if (is_resonance_nm(n,m,nphi)) then
	         ires = ires + 1
	         nres(ires) = n
		 mres(ires) = m
                 nphires(ires) = nphi
      ...
      ! --------------------------------!
       ...
       do n = nkx1, nkx2
          do m = nky1, nky2
             cexp0 = xx(n, i_global) * yy(m, j_global) * zz(nphi, k_global) &
     &           * zbeta_iharm(n,m,nphi)
	     cexp1 = cexp0 * zbeta(n,m,nphi)
	     cexp2 = cexp0 / zbeta(n,m,nphi)
		
             epsx = isq2 * (ealphak(n, m, nphi) - zi * ebetak(n, m, nphi)) * cexp1
             epsy = isq2 * (ealphak(n, m, nphi) + zi * ebetak(n, m, nphi)) * cexp2
             epsz = ebk(n, m, nphi) * cexp0
	     
             sum2_1 = sum2_1 + conjg(epsx) * Jn(IHARM + 1, n, m, nphi)
             sum2_2 = sum2_2 + conjg(epsy) * Jn(IHARM - 1, n, m, nphi)
             sum2_3 = sum2_3 + conjg(epsz) * Jn(IHARM, n, m, nphi)
          end do
       end do
       end do
  1000 continue
...
!          cexp2 = xx(n, i_global) * yy(m, j_global) * zbeta_iharm(n,m) / zbeta(n,m)
     ...
     &         * zbeta_iharm(n,m, nphi)
	  ...
	  sumkx2_1 = sumkx2_1 + xkxsav(n) * conjg(epsx) * Jn(IHARM + 1, n, m, nphi)
          sumkx2_2 = sumkx2_2 + xkxsav(n) * conjg(epsy) * Jn(IHARM - 1, n, m, nphi)
          sumkx2_3 = sumkx2_3 + xkxsav(n) * conjg(epsz) * Jn(IHARM, n, m, nphi)
	     
	  sumky2_1 = sumky2_1 + xkysav(m) * conjg(epsx) * Jn(IHARM + 1, n, m, nphi)
          sumky2_2 = sumky2_2 + xkysav(m) * conjg(epsy) * Jn(IHARM - 1, n, m, nphi)
          sumky2_3 = sumky2_3 + xkysav(m) * conjg(epsz) * Jn(IHARM, n, m, nphi)

          UPAR0 = SQMUT0 / NPARA_sav(n, m, nphi) * (1. - NWCW)
	  ...
	  dfduper0 = dfduper(k_uper, NUPAR)
          ...
          factor = PISQMUT0 / abs(NPARA_sav(n, m, nphi)) 
			
          sumb_11_nm = UPER(k_uper) * UPER(k_uper)  * Jn(IHARM + 1, n, m, nphi) * epsx     &
     &               + UPER(k_uper) * UPER(k_uper)  * Jn(IHARM - 1, n, m, nphi) * epsy     &
     &               + SQ2 * UPER(k_uper) * UPAR0  * Jn(IHARM, n, m, nphi)     * epsz

          sumb_31_nm = SQ2 * UPER(k_uper) * UPAR0  * Jn(IHARM + 1, n, m, nphi) * epsx     &
     &               + SQ2 * UPER(k_uper) * UPAR0  * Jn(IHARM - 1, n, m, nphi) * epsy     &
     &               + 2.0 * UPAR0 * UPAR0 * Jn(IHARM, n, m, nphi)     * epsz
...
!          cexp2 = xx(n, i_global) * yy(m, j_global) * zbeta_iharm(n,m) / zbeta(n,m)
     ...
     &         * zbeta_iharm(n,m, nphi)
	  ...
	  dfduper0 = dfduper(k_uper, NUPAR)
          ...
          DFACTPAR = NPARA_sav(n, m, nphi) * UPAR0 * SQMUT0I
          DFACTPER = NPARA_sav(n, m, nphi) * UPER(k_uper) * SQMUT0I
